...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.motor_mu_fric = torch.zeros(self.num_envs, dtype=torch.float, device=self.device) # dynamic friction coefficient
        self.motor_Fstatic_fric = torch.zeros(self.num_envs, dtype=torch.float, device=self.device) # static friction coefficient
        self.use_random_fric = True
    ...
    def _apply_action(self) -> None:
        ...
        # Add frictional torques if enabled
        if self.use_random_fric:
            friction_torques = self.motor_mu_fric.unsqueeze(1) * self.robot.data.joint_vel + \
                                self.motor_Fstatic_fric.unsqueeze(1) * torch.tanh(self.robot.data.joint_vel / 0.01)
            torques -= friction_torques
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # First derivative (Current - Last), shape: (num_envs,)
        ...
        rew_ang_vel_xy = torch.sum(torch.square(self.robot.data.root_ang_vel_b[:, :2]), dim=1) * self.cfg.ang_vel_xy_reward_scale
        ...
        # 5. Foot clearance penalization
        rew_feet_clearance = self._reward_feet_clearance()
        # 6. Foot contact tracking penalization
        rew_contact_tracking = self._reward_tracking_contact_forces()
        rew_raibert_heuristic = self._reward_raibert_heuristic()
        # 7. Torque penalty
        rew_torque_penalty = torch.sum(torch.square(self._actions), dim=1)
        # Add to rewards dict
        rewards = {
            ...
            "tracking_contacts_shaped_force": rew_contact_tracking * self.cfg.tracking_contacts_shaped_force_reward_scale,
            "torque_penalty": rew_torque_penalty * self.cfg.torque_penalty_scale,
        }
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # resample friction coefficients if enabled
        if self.use_random_fric:
            self.motor_mu_fric[env_ids] = torch.zeros_like(self.motor_mu_fric[env_ids]).uniform_(0.0, 0.3)
            self.motor_Fstatic_fric[env_ids] = torch.zeros_like(self.motor_Fstatic_fric[env_ids]).uniform_(0.0, 2.5)
        # Sample new commands
    ...
    def _reward_feet_clearance(self):
        # adapt the implementation from IsaacGymEnvs.
        # compute foot height errors according to gait desired_contact_states
        phases = torch.abs(1.0 - (self.foot_indices * 2.0)) * 1.0 - 0.5
        ...
        rew_foot_clearance = torch.square(target_height - foot_height) * (
            1 - self.desired_contact_states) / 4.0# + torch.square(foot_height - 0.02) * self.desired_contact_states / 4.0
        # shape : (num_envs, )
        return torch.sum(rew_foot_clearance, dim=1)
        
    def _reward_tracking_contact_forces(self):
        # adapt the implementation from IsaacGymEnvs.
        net_contact_forces = self._contact_sensor.data.net_forces_w
        foot_contact_forces = torch.norm(net_contact_forces[:, self._feet_ids_sensor, :], dim=-1)
        rew_tracking_contacts_shaped_force = torch.zeros(self.num_envs, device=self.device)
        for i in range(4):
            # penalize when foot is supposed to be in the air but there is contact force
            rew_tracking_contacts_shaped_force += - (1 - self.desired_contact_states[:, i]) * (
                    1 - torch.exp(-1 * foot_contact_forces[:, i] ** 2 / 100.))
        # shape: (num_envs,)
